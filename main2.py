...
class OptionWindow(tk.Toplevel):
    def __init__(self, parent, app_instance):
        ...
        self.option_type_var.set("Generate Binary Response")
        ...
        self.option_type_dropdown['values'] = ["Generate Binary Response", "Generate Multiple Response", "Generate Weighted Response"]
    ...
    def create_option(self):
        ...
        if option_type == "Generate Binary Response":
            self.app_instance.add_option(option_type)
        elif option_type == "Generate Multiple Response":
            self.app_instance.add_multiple_response_option()
        elif option_type == "Generate Weighted Response":
            self.app_instance.add_weighted_response_option()

        self.destroy()

class DefineChoicesWindow(tk.Toplevel):
    def __init__(self, parent, name_entry, standalone=False):
        super().__init__(parent)
        self.title("Define Choices")
        self.parent = parent
        self.name_entry = name_entry
        self.standalone = standalone
        self.generator = WeightedChoiceGenerator(standalone)
    ...
    def finish(self):
        if self.standalone:
            ...
            self.name_entry.weighted_choices = self.generator.choices
        self.destroy()
...
class SyntheticDataGenerator:
    ...
    def add_multiple_response_option(self):
        var = tk.StringVar(value="0")
        checkbox = tk.Checkbutton(self.entry_frame, variable=var)
        checkbox.grid(row=len(self.options), column=0, padx=5, pady=5)
        self.checkboxes.append(checkbox)

        label = tk.Label(self.entry_frame, text="Generate Multiple Response")
        label.grid(row=len(self.options), column=1, padx=5, pady=5)
        ...
        multiple_response_button = tk.Button(self.entry_frame, text="Define Choices", command=lambda: self.define_multiple_choices(name_entry))
        multiple_response_button.grid(row=len(self.options), column=4, padx=5, pady=5)

        self.options.append(("Generate Multiple Response", var, name_entry))

    def add_weighted_response_option(self):
        var = tk.StringVar(value="0")
        checkbox = tk.Checkbutton(self.entry_frame, variable=var)
        checkbox.grid(row=len(self.options), column=0, padx=5, pady=5)
        self.checkboxes.append(checkbox)

        label = tk.Label(self.entry_frame, text="Generate Weighted Response")
        label.grid(row=len(self.options), column=1, padx=5, pady=5)
        ...
        weighted_response_button = tk.Button(self.entry_frame, text="Define Choices", command=lambda: self.define_weighted_choices(name_entry))
        weighted_response_button.grid(row=len(self.options), column=4, padx=5, pady=5)

        self.options.append(("Generate Weighted Response", var, name_entry))

    def define_multiple_choices(self, name_entry):
        DefineChoicesWindow(self.root, name_entry)

    def define_weighted_choices(self, name_entry):
        DefineChoicesWindow(self.root, name_entry, standalone=True)
    ...
    def update_options_display(self):
        ...
        for i, (option, _, *entries) in enumerate(self.options):
            ...
            if option == "Generate Binary Response":
            ...
            elif option == "Generate Multiple Response" or option == "Generate Weighted Response":
                name_entry = entries[0]
                name_entry.grid(row=i, column=2, padx=5, pady=5)
                define_choices_button = tk.Button(self.entry_frame, text="Define Choices", command=lambda ne=name_entry: self.define_weighted_choices(ne) if option == "Generate Weighted Response" else self.define_multiple_choices(ne))
                define_choices_button.grid(row=i, column=4, padx=5, pady=5)

                view_choices_button = tk.Button(self.entry_frame, text="View Choices", command=lambda ne=name_entry: self.view_multiple_choices(ne))
                view_choices_button.grid(row=i, column=5, padx=5, pady=5)

    def view_multiple_choices(self, name_entry):
    ...
    def generate_weighted_choice(self, choices, standalone=False):
        if standalone:
    ...
    def generate_data(self):
        ...
        for _ in range(num_entries):
            ...
            for option_type, _, *entries in self.options:
                ...
                elif option_type == "Generate Multiple Response" or option_type == "Generate Weighted Response":
                    entry_name = entries[0].get()
                    weighted_choices_generator = entries[0].weighted_choices
                    if weighted_choices_generator is None:
                        messagebox.showerror("Error", "Weighted choices not defined.")
                        return
                    standalone = option_type == "Generate Weighted Response"
                    if isinstance(weighted_choices_generator, WeightedChoiceGenerator):
                        entry_data[entry_name] = weighted_choices_generator.generate_weighted_choice(standalone=standalone)
                    else:
                        entry_data[entry_name] = self.generate_weighted_choice(weighted_choices_generator, standalone=standalone)
...
class WeightedChoiceGenerator:
    ...
    def generate_weighted_choice(self, standalone=False):
        if self.standalone:
