...
class SurveyGeneratorApp:
    def __init__(self, root):
        ...
        # Set app icon (leave blank for now)
    ...
    def add_weighted_choice_option(self):
        ...
        view_choices_button.grid(row=len(self.options), column=5, padx=5, pady=5)

        self.options.append(("Generate Weighted Choice", var, name_entry))

    def define_weighted_choices(self, name_entry):
        generator = WeightedChoiceGenerator(self)
        while True:
            choice = simpledialog.askstring("Weighted Choice", "Enter choice:")
            if choice is None:
                break
            probability = simpledialog.askstring("Weighted Choice", f"Enter probability for {choice}:")
            ...
            try:
                ...
                if not 0 <= probability <= 1:
                    ...
                    continue
                generator.add_choice_entry(choice, probability)
            except ValueError:
                messagebox.showerror("Error", "Invalid probability value")
        name_entry.weighted_choices = generator
        total_prob = sum(generator.choices.values())
        if total_prob != 1.0:
            messagebox.showerror("Error", "Total probability must equal 1")
        else:
            finish_button = tk.Button(self.entry_frame, text="Finish", command=self.root.quit)
            finish_button.grid(row=len(self.options), column=6, padx=5, pady=5)

    def view_weighted_choices(self, name_entry):
        if hasattr(name_entry, 'weighted_choices'):
            choices = name_entry.weighted_choices.choices
            choices_list = "\n".join([f"{choice}: {prob}" for choice, prob in choices.items()])
            messagebox.showinfo("Weighted Choices", choices_list)
        else:
            messagebox.showinfo("Weighted Choices", "No choices defined.")
