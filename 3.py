        ...
        for _ in range(num_entries):
            ...
            for option_type, _, *entries in self.options:
                if option_type == "Generate Response":
                    ...
                    weighted_choice_str = simpledialog.askstring("Weighted Choices", f"Enter weighted choices for '{entry_name}':")
                    if weighted_choice_str is None:
                        return
                    weighted_choice_dict = eval(weighted_choice_str)
                    entry_data['Entry Name'] = entry_name
                    entry_data['Response'] = self.generate_weighted_choice(weighted_choice_dict)
