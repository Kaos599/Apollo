...
class SurveyGeneratorApp:
    ...
    def delete_selected(self):
        ...
        for i, option_tuple in enumerate(self.options):
            if len(option_tuple) > 1:
                ...
                print("Tuple causing error:", option_tuple)
    ...
    def get_weighted_choices(self):
        weighted_choice_str = simpledialog.askstring("Weighted Choices", "Enter weighted choices in the format 'Option:Probability', separated by commas:")
        if weighted_choice_str is None:
            return None

        weighted_choices = {}
        total_probability = 0

        for choice_prob_pair in weighted_choice_str.split(','):
            # Splitting the choice and probability pair by colon ':'
            choice, prob = map(str.strip, choice_prob_pair.split(':'))
    ...
    def generate_data(self):
        ...
        for _ in range(num_entries):
            entry_data = {'Entry Name': '', 'Response': ''}  # Collect data for each entry here
        ...
        selected_choice = self.generate_weighted_choice(weighted_choices)
        print("Selected Choice:", selected_choice)
        entry_data['Response'] = selected_choice
